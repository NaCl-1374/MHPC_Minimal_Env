...
#else
  #define CASADI_PREFIX(ID) Jacob_B_ ## ID
#endif
...
#ifdef MATLAB_MEX_FILE
void mex_Jacob_B(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  casadi_real w[76];
  casadi_int *iw = 0;
  const casadi_real* arg[1] = {0};
  casadi_real* res[2] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Jacob_B\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Jacob_B\" failed. Too many output arguments (%d, max 2)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+28);
  --resc;
  res[0] = w+14;
  if (--resc>=0) res[1] = w+22;
  i = Jacob_B(arg, res, iw, w+28, 0);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Jacob_B\" failed.");
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_Jacob_B(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "Jacob_B")==0) {
    mex_Jacob_B(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'Jacob_B'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
